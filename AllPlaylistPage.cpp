...
void MainWindow::LoadAllPlaylistPage() { // it's called that
    ...
    for(odb::result<Playlist>::iterator it = playlist_ptr.begin(); it != playlist_ptr.end(); it++){
        ...
        */
        ...
        odb::result<Track_Playlist> track_mapping = database_context.query<Track_Playlist>(odb::query<Track_Playlist>::id == playlist.Id());
        ...
        Track_Playlist track_playlist = *(Track_Playlist*)(track_playlist);
        // Retrieve the track from the playlist
        Track* track_ = database_context.query<Track>(odb::query<Track>::id == track_playlist.TrackId());
        ...
        QStandardItem* playlist_card = new QStandardItem(
            QString::fromStdString(
                std::string(playlist_name + " (" + playlist_year + ")" + "\n" 
                            + "Length: " + std::string(playlist_duration/60)
                            )
                )
        )
    }
